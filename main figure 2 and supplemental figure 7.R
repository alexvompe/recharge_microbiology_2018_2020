...
# mean daily temperature at LTER 0
...
##Start here for mean daily temperature and heat stress calculations for LTER 0 and 2 average for
##Histories of disturbance
temperature$day=as.Date(temperature$day, '%Y-%m-%d')

temperature$day=ymd(temperature$day)

lter.day.all <- temperature %>% group_by(day) %>% summarise(temp_c = mean(temperature_c)) %>% ungroup()

lter.day.all$day<-as.Date(lter.day.all$day, '%Y-%m-%d')

lter.day.all$day <- ymd(lter.day.all$day)

lter.day.all$day <- as.Date(lter.day.all$day, '%Y-%m-%d')

lter.time.seq.all <- data.frame(day=unique(lter.day.all$day),ind=seq(1:length(unique(lter.day.all$day))))
lter.time.seq.all$year <- year(lter.time.seq.all$day)
lter.time.seq.all$week.num <- week(lter.time.seq.all$day)
lter.time.seq.week.all <- lter.time.seq.all %>% group_by(week.num,year) %>% summarise(day=min(day)) %>% ungroup()

lter.day.temp.all<-left_join(lter.day.all, lter.time.seq.all, by='day')
lter.week.temp.all<- lter.day.temp.all %>% group_by(year,week.num) %>% summarise(temp_c = mean(temp_c)) %>% ungroup()
lter.week.temp.all <- lter.week.temp.all[with(lter.week.temp.all, order(year,week.num)),]

mma_ref <- 29

# calculate accumulated heat stress
lter.week.temp.all$hotspot <- lter.week.temp.all$temp_c - mma_ref
lter.week.temp.all$hotspot[lter.week.temp.all$hotspot < 0] <- 0

lter.week.temp.all$cumstress <- NA

# 12 week running sum 
lter.out.all <- lter.week.temp.all 
for(i in 13:nrow(lter.out)){
  lter.out.all$cumstress[i] <- sum(lter.out.all$hotspot[(i-12):i],na.rm=T)
}

lter.out.all$cumstress[is.na(lter.out.all$cumstress)] <- 0
lter.out.all <- left_join(lter.out.all, lter.time.seq.week.all, by=c('year','week.num'))

head(lter.out.all)
sum(is.na(lter.out.all$temp_c))
write.csv(temperature, "all temp data check.csv")#none 0 or NA, so there aren't any data gaps

##Figure 2 plot history of disturbance Panel 1
lter.out_hist_subset = subset(lter.out.all, day >= '2009-12-01' & day <= '2021-01-01')
...
p4=ggplot(lter.out.all, aes(x=day, y=cumstress))+
...
ggsave(plot=p4, filename="updated full disturbance history supplemental.tiff", scale=2,
...
##Make figure 2. Additional annotations added in powerpoint.
p = ggarrange(p1,p2,p3, ncol=1)
ggsave(plot=p, filename="updated main figure 2.tiff", scale=3,
