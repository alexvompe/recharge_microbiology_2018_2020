...
p3=ggplot(ord_plob_df, aes(x=Axis.1, y=Axis.2, color=percent_dead>0,
                          linetype=percent_dead>0)) + 
  theme_bw()+
  geom_point(size=3, alpha = 0.6)+
  geom_point(aes(x=mean(ord_acr_df$Axis.1[ord_plob_df$percent_dead==0]), 
                 y=mean(ord_acr_df$Axis.2[ord_plob_df$percent_dead==0])), size=8,
             color="black")+
  geom_point(aes(x=mean(ord_acr_df$Axis.1[ord_plob_df$percent_dead>0]), 
                 y=mean(ord_acr_df$Axis.2[ord_plob_df$percent_dead>0])), size=8,
             color="darkred")+
...
p4=ggplot(ord_acr_df, aes(x=Date, y=Axis.1, color=percent_dead, group=1)) + 
  theme_bw()+
...
p5=ggplot(ord_acr_df, aes(x=Date, y=Axis.1, color=percent_bleached, group=1)) + 
  theme_bw()+
  geom_point(size=3, position="jitter")+
  geom_hline(yintercept=0, linetype="dashed")+
  geom_smooth(aes(as.numeric(Date), Axis.1, group=1), method = "loess",
...
p6 = ggplot(aug, aes(x=binary_mortality, y=log(mort_disp)))+
  theme_bw()+
  geom_point(alpha = 0.8, color = "#56B4E9", size = 3)+
  geom_boxplot(color = "#56B4E9", linewidth = 1.2, alpha = 0)+
...
p = ggarrange(p1, p5, p2, p4, p3, p6, ncol=2, nrow=3)
ggsave(plot=p, "main figure 8 intermediate updated.tiff",height = 200,
